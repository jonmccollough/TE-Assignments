...
SELECT name, lifeexpectancy FROM country WHERE continent = 'Oceania' AND lifeexpectancy NOTNULL;
...
SELECT name, ((gnp*1000000) / population) AS perCapitaGNP FROM country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, (2020 - indepyear) AS yearsOfIndep FROM country WHERE indepyear NOTNULL;
